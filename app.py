...
if page_choice == "1. DPSAC Tracker":
    ...
    # --- UNIT STATUS METRICS ---
    s_col = next((c for c in master_df.columns if c.lower() == 'unit status'), "Unit Status")
    ...
    # --- SIDEBAR STATUS FILTER (Request Restored) ---
    st.sidebar.markdown("---")
    st.sidebar.subheader("🔍 Filter by Status")
    status_choice = st.sidebar.selectbox("Generate Detailed List:", ["None", "Active", "Shifted", "Sold"], key="sidebar_status")
    ...
    if status_choice != "None":
        ...
        if not f_list.empty:
            st.write(f"Total Found: {len(f_list)}")
    ...
    with tabs[0]: # Machine Tracker
        ...
        if sel_f != "Select":
            ...
            st.divider()
            f_match = foc_df[foc_df['FABRICATION NO'].astype(str) == sel_f] if 'FABRICATION NO' in foc_df.columns else pd.DataFrame()
    ...
    with tabs[1]: # FOC List
        ...
        f_list_std = foc_df[foc_df['FABRICATION NO'].astype(str).isin(std_fabs)] if 'FABRICATION NO' in foc_df.columns else pd.DataFrame()
...
# ==========================================
    ...
    s_col_i = next((c for c in master_od_df.columns if c.lower() == 'unit status'), "Unit Status")
    ...
    with tabs_i[0]: # Machine Tracker
        ...
        sel_c_i = col1_i.selectbox("Select Customer (IND)", ["All"] + c_l_i, key="ind_c_sel")
        ...
        if sel_f_i != "Select":
            ...
            st.divider()
            f_match_i = foc_df[foc_df['FABRICATION NO'].astype(str) == sel_f_i] if 'FABRICATION NO' in foc_df.columns else pd.DataFrame()
            st.subheader("🎁 Machine FOC Details")
            ...
            st.subheader("🕒 Service History")
            hi_m_ind = service_df[service_df['Fabrication Number'].astype(str) == sel_f_i].sort_values(by='Call Logged Date', ascending=False)
    ...
    with tabs_i[1]: # Industrial FOC List
        ...
        f_list_i = foc_df[foc_df['FABRICATION NO'].astype(str).isin(ind_fabs)] if 'FABRICATION NO' in foc_df.columns else pd.DataFrame()
    ...
    with tabs_i[2]: # Industrial Pending
        ...
        if not pi_df.empty:
            ...
            st.dataframe(pi_df, use_container_width=True)import streamlit as st
import pandas as pd
import os
from datetime import datetime
from io import BytesIO
