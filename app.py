...
if page_choice == "1. DPSAC Tracker":
    # --- NEW SIDEBAR FILTER FOR UNIT STATUS ---
    st.sidebar.markdown("---")
    st.sidebar.subheader("🔍 Filter by Status")
    status_filter = st.sidebar.selectbox(
        "Select Status to View List:",
        ["None", "Active", "Shifted", "Sold"]
    )

    if status_filter != "None":
        st.subheader(f"📋 {status_filter} Machines List")
        # Filter data based on selection
        filtered_status_df = master_df[master_df[s_col].astype(str).str.contains(status_filter, case=False, na=False)]
        
        if not filtered_status_df.empty:
            st.write(f"**Found {len(filtered_status_df)} machines.**")
            # Display important columns only
            display_cols = ['Fabrication No', 'CUSTOMER NAME', 'MODEL', 'Unit Status', 'HMR Cal.']
            st.dataframe(filtered_status_df[display_cols], use_container_width=True)
            
            # Export Option for filtered list
            st.download_button(
                label=f"📥 Download {status_filter} List",
                data=to_excel(filtered_status_df),
                file_name=f"DPSAC_{status_filter}_List.xlsx"
            )
        else:
            st.warning(f"No machines found with status: {status_filter}")
        st.divider()
